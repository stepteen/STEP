...
public class RegisterActivity extends Activity implements View.OnClickListener{
    ...
    private void initView() {
        dlgLoading=new DlgLoading(RegisterActivity.this);
    ...
    private void postAsynHttp(String zhanghao,String mima) {
        String url=ContractData.URL+"/user/register";
        user.setPhoneNum(zhanghao);
        user.setUserPwd(Md5.digest(mima.getBytes()));
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                dlgLoading.dismiss();
                if(parseResultData(response.body().string())) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
